    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i != numOfCommodities; i++) {
            ...
            for (String current : nodes) {
                if (current.equals(currentCommodity_source) || current.equals(currentCommodity_sink))
                    continue;

                if (adj_list_in.containsKey(current) && adj_list_out.containsKey(current)) {
                    GRBLinExpr expr_inFlow = new GRBLinExpr();
                    GRBLinExpr expr_outFlow = new GRBLinExpr();

                    List<String> tmp_outFlow = adj_list_out.get(current);
                    ...
                    model.addConstr(expr_inFlow, GRB.EQUAL, expr_outFlow, "c1_" + current + "-" + i);

                } else if (adj_list_in.containsKey(current) && !adj_list_out.containsKey(current)) {
                    GRBLinExpr expr_inFlow = new GRBLinExpr();
                    List<String> tmp_inFlow = adj_list_in.get(current);
                    for (int k = 0; k != tmp_inFlow.size(); k++) {
                        String from = tmp_inFlow.get(k);
                    ...
                    model.addConstr(expr_inFlow, GRB.EQUAL, 0, "c1_" + current + "-" + i);
                    ...
                    model.addConstr(expr_outFlow, GRB.EQUAL, 0, "c1_" + current + "-" + i);
                }
