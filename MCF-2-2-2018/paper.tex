...
We tested with different commodity sizes to find out how the proposed algorithm perform under various settings. More specifically, Table \ref{tab:results} shows the settings with comparison between random rounding and LP solution, each setting is ran for 10 times and average is reported.

\begin{table}
	\centerline{
		\begin{tabular}{|c|c|c|}\hline
			Commodity Size & Random Rounding  & LP  \\ \hline
            1000 & 50 & 50 \\ \hline
            3000 & 50 & 49.96 \\ \hline             
            5000 & 47.5 & 47.51 \\ \hline
            7000 & 42 & 42.13 \\ \hline
            9000 & 38.2 & 38.31 \\ \hline
		\end{tabular}
	}
    \vspace{0.1in}
	\caption{Average number of commodity received for different commodity sizes}
	\label{tab:results}
\end{table}

Figure \ref{fig:edgeload7000} shows the edge load when commodity size is 7000MB, basically as we can see from the figure that most of the edges have little load, and very few edges have moderate load and few edges are fully saturated. This is due to the reason that some connections are not used for delivering commodities and some connections on the main path are used for delivery of many commodities. 

\begin{figure}
	\begin{center}
		\psfig{figure=fig/EdgeLoad_C50_F7000.png, height=2.5in, width=3.6in}
		\caption{Edge Load for Commodity Size 7000MB}
		\label{fig:edgeload7000}
	\end{center}
\end{figure}


Figure \ref{fig:ratio} shows the ratio of number of commodities received by RR and LP, which is a better view from Table \ref{tab:results}, as we can see, even though different commodity size results in a different number of commodity received, the ratio stays around 1 which means RR is relatively competitive as LP solution.

\begin{figure}
	\begin{center}
		\psfig{figure=fig/ratio.png, height=1.8in}
		\caption{RR/LP Ratio in terms of commodity delivery}
		\label{fig:ratio}
	\end{center}
\end{figure}
...
\begin{figure}
	\begin{center}
		\psfig{figure=fig/violation.png, height=2.5in, width=3.6in}
...
\begin{figure}
	\begin{center}
		\psfig{figure=fig/obj_value.png, height=2.5in, width=3.6in}
...
\begin{figure}
	\begin{center}
		\psfig{figure=fig/best.png, height=2.5in, width=3.6in}
